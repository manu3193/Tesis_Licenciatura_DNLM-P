...
En este cap\'itulo se procede a detallar el contenido te\'orico necesario para el desarrollo de la propuesta. Como se presenta en los cap\'itulos anteriores, la propuesta consiste en la paralelizaci\'on de un filtro que combina la eliminaci\'on de ruido y la mejora de la imagen. La reducci\'on de ruido se realiza por medio del enfoque de filtrado espacial, mediante el ponderamiento de la similitud entre las regiones aleda\~nas a los pixeles de la imagen. Este enfoque presenta una gran robustez ante el ruido, pero una mayor complejidad computacional, la cual se aborda con mayor detalle en esta secci\'on. La mejora de la imagen se emplea mediante un enfoque de Unsharp Masking, permitiendo un mayor realce en bordes y contraste en regiones de la imagen. Adem\'as se presentan t\'ecnicas de paralelizaci\'on que se adec\'uan al enfoque del filtro, con el fin de reducir el tiempo de procesamiento.
...
Los m\'etodos de filtrado discutidos en este cap\'itulo se realizan en el dominio espacial. El dominio espacial es el plano que contiene los valores de los pixeles de una imagen, representada por una matriz de pixeles. Las t\'ecnicas en el dominio espacial realizan operaciones directamente en la matriz de pixeles, a diferencia de otros m\'etodos basados en transformaciones (transformada de Fourier, Wavelets e histogramas) con el fin de realizar las operaciones en otros dominios. 

\subsubsection{Filtros lineales y filtros no lineales}



\subsection{Agudizamiento de detalles en imagen}\
