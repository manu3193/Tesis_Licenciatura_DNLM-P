...
En este cap\'itulo se detalla el contenido te\'orico necesario para el desarrollo de la propuesta presentada en ested trabajo. La propuesta consiste en la paralelizaci\'on de un filtro que combina la eliminaci\'on de ruido y la mejora de la imagen. La reducci\'on de ruido se realiza por medio de un enfoque de filtrado espacial no local, mediante el ponderamiento de la similitud entre las regiones aleda\~nas a los pixeles de la imagen. Este enfoque presenta una gran robustez ante el ruido, pero una mayor complejidad computacional, la cual se aborda con mayor detalle en este cap\'itulo. La mejora de la imagen se realiza utilizando uno de los enfoques de Unsharp Masking, permitiendo un mayor realce en bordes y contraste en regiones de la imagen. Adem\'as se presentan t\'ecnicas de paralelizaci\'on que permitan el enfoque del desacople entre la imagen de filtrado y la utilizada para el pesado, como se propone en \cite{deceived}., con el fin de reducir el tiempo de procesamiento del algoritmo en una primera instancia y evitar los artefactos que resultan al utilizar el enfoque tradicional de filtrado en etapas sucesivas. 
